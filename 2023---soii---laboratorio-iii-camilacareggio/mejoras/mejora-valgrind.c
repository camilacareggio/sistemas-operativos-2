...
void compute(double** arr, int kern[3][3]){
    ...
    double tmp_sum[9];
    double dato, accum=0; // initialize accum to prevent valgrind errors
    int i, j, k, l;
    for(i = 0 ; i < XDIM ; i++)
        for(j = 0 ; j < YDIM ; j++){
            ...
            if(i >= 1 && j >=1 && i < XDIM-1 && j <YDIM-1){
                ...
                accum = 0; // accum is initialized inside the if statement
                for(k = 0; k < 3; k++)
                    for(l = 0; l < 3; l++)
                        accum = accum + tmp_sum[k*3+l];
  
            }
            arr[i][j] = accum; // accum is used outside the if statement
        }    
}
...
int main(void)
{
    ...
    for (int i = 0; i < XDIM; i++) { // free allocated space
        free(arr[i]);
    }
    free(arr);
